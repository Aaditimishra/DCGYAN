<?php include 'check.php'; $pageurl="review"; $tbl_name="dc_certificate"; $pagename="Student Review"; $mintablewidth="1000px";
...
$sql="UPDATE `$tbl_name` SET hide='1' WHERE ID='$companyasend'";
if (!mysqli_query($con,$sql)){die('Error: ' . mysqli_error()); }
echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?page=$_GET[page]&message=Certificate deletd..'>";  exit(0);
...
if (!mysqli_query($con,$sql)) {die('Error: ' . mysqli_error()); }
echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?page=$_GET[page]&message=Certificate updated..'>";  exit(0);
...
} else if ($_GET['mode'] == "status" && $_GET['id']!="") {
$sql="UPDATE `$tbl_name` SET status ='$_GET[status]' WHERE sessionid='$_GET[id]'";
if (!mysqli_query($con,$sql)) {die('Error: ' . mysqli_error()); }
echo"<META HTTP-EQUIV='REFRESH' CONTENT='0; URL=$pageurl?page=$_GET[page]&message=Certificate updated..'>";  exit(0);  
} else { ?>
...
<table width="100%" border="0">
  <tr>
    <td><a onClick="popupCenter('addcertificate<?php echo $extn ?>', 'myPop1',550,550);" href="javascript:void(0);" style="text-decoration:none;color:#00F; cursor:pointer;"><span class="btn btn-primary btn-sm"><i class="fa fa-plus-square"></i> ADD NEW REVIEW</span></a></td>
...
<div style='overflow-x: scroll;'> 
	...
	*/
    ...
    if($_GET['search']!=""){
    $query = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE hide='0' and (name like '%$search%' or fname like '%$search%' or contact like '%$search%' or enrollment like '%$search%')";	
    $sqlx = "SELECT * FROM `$tbl_name` WHERE hide='0' and (name like '%$search%' or fname like '%$search%' or contact like '%$search%' or enrollment like '%$search%')"; 

	} else {
	
	$query = "SELECT COUNT(*) as num FROM `$tbl_name` WHERE hide='0'";	
    $sqlx = "SELECT * FROM `$tbl_name` WHERE hide='0'";    }
...
?>			 
	...
	<?php if ($total_pages>="1"){?> 
		<form name="delete" id="frmCompare" class="frmCompare" action="<?php echo $pageurl ?>?page=<?php echo $_GET['page'] ?>" method="post"> 
...
echo "<td width='40'><input type='checkbox' id='selectall' title='Select All' style='cursor:pointer;'/></td>";
echo "<td width='90' align='left' style='padding-left:10px;'>Status</td>";
echo "<td width='80'>Photo</td>";
echo "<td align='left' style='padding-left:10px;'>Student Name</td>";
echo "<td align='left' style='padding-left:10px;'>Review</td>";
echo "<td width='175'>Uploaded ON</td>";

echo "</tr>";

$in = 0;
while ($row = mysqli_fetch_array($result)) {
    if (!empty($row['review'])) { // Check if 'review' is not empty or null
        $in = $in + 1;
        if ($bgcolor == '#F8F8FF') {
            $bgcolor = '#ffffff';
        } else {
        ...
        echo "<td><b>$tnum</b></td>";
        echo "<td><input type='checkbox' class='td' name='ids[]' value='$row[ID]' style='cursor:pointer;'></td>";
        echo "<td align='left' style='padding-left:10px;'>";
        if ($row['status'] == "1") {
            ?><a href="<?php echo $pageurl ?>?mode=status&status=0&id=<?php echo $row['sessionid'] ?>" style="color:#009900">Active</a><?php
        } else {
            ?><a href="<?php echo $pageurl ?>?mode=status&status=1&id=<?php echo $row['sessionid'] ?>" style="color:#FF0000">Inactive</a><?php
        }
        echo "</td>";
        echo "<td><a href='images/pdf/$row[pic]' style='color:#0000FF;' target='_blank'>View</a></td>";
        echo "<td align='left' style='padding-left:10px;'><b>$row[name]</b>";
        ?><a onClick="popupCenter('addcertificate<?php echo $extn ?>?id=<?php echo $row['sessionid'] ?>', 'myPop1',550,600);" href="javascript:void(0);" style="text-decoration:none;color:#00F; cursor:pointer;"><img src='images/edit.png' title='Edit'></a><?php
        echo "</td>";
        echo "<td align='left' style='padding-left:10px;'>$row[review]</td>";
